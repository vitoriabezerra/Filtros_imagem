...
public class HarrisCornerDetector {
	...
	public static final float DEFAULT_ALPHA = 0.04f;
	...
	final float[] bfilt = { 0.01563f, 0.09375f, 0.234375f, 0.3125f, 0.234375f, 0.09375f, 0.01563f }; // = [1, 6, 15, 20,
																										// 15, 6, 1]/64
	...
	List<Corner> collectCorners(int border) {
		...
		Collections.sort(cornerList); // pediu para extrair do metodo.
		//System.out.println(cornerList.size());
		return cornerList;
	}
	...
	List<Corner> cleanupCorners(List<Corner> corners) {
		...
		for (int i = 0; i < cornerArray.length; i++) {
			if (cornerArray[i] != null) {
				...
				goodCorners.add(c1); // delete all remaining corners close to c
		...
		//System.out.println(goodCorners.size());
		return goodCorners;
	...
	static boolean isLocalMax(FloatProcessor fp, int u, int v) {
		...
		if (u <= 0 || u >= w - 1 || v <= 0 || v >= h - 1)
			...
			float cp = pix[i1];
			return cp > pix[i0 - 1] && cp > pix[i0] && cp > pix[i0 + 1] && cp > pix[i1 - 1] && cp > pix[i1 + 1]
